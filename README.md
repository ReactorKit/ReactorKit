...
Define `scheduler` property to specify which scheduler is used for observing the state stream. Note that this queue **must be** a serial queue. The default scheduler is `CurrentThreadScheduler`.
